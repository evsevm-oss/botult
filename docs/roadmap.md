...
- [ ] Прием сообщений в чате как есть: пользователь отправляет блюда обычными сообщениями
  - [ ] Текст: нормализация через `/api/normalize`, превью и подтверждение (быстрые кнопки «Сохранить/Отменить/Править»)
  - [ ] Фото: принять фото, создать черновик приема, поставить в очередь на распознавание (этап 9), дать ручное заполнение и подтверждение
  - [ ] Аудио/голос: распознавание речи (STT, Whisper/OpenAI) → нормализация → подтверждение
- [ ] REST дневника: CRUD эндпоинты
  - [ ] GET `/api/meals?date=YYYY-MM-DD` и GET `/api/meals/{id}`
  - [ ] POST `/api/meals` (создание приема с `items`), PATCH `/api/meals/{id}`, DELETE `/api/meals/{id}`
- [ ] Бизнес-логика дневника
  - [ ] Обновление `daily_summary` после любых CRUD операций
  - [ ] Идемпотентность: защита от повторной записи по `update_id/message_id`
  - [ ] Валидация значений (масса, единицы, БЖУ) и дружественные ошибки
- [ ] Производительность и кэш
  - [ ] Повторное использование кэша нормализации (Redis) для одинаковых сообщений
- [ ] Логи и метрики
  - [ ] trace_id сквозной; метрики: доля авто‑подтверждений vs. требующих уточнения, среднее число позиций
- [ ] WebApp (дневник)
  - [ ] Список приемов за день, просмотр позиций, редактирование/удаление
  - [ ] Быстрое добавление/редактирование позиций (тонкие UI для сложных блюд)
