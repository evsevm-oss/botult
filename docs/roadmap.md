...
- [ ] Разработать брендинг: логотип для аватарки Telegram (палитра, светлая/темная версии), экспорт ассетов (SVG/PNG 640×640/512×512/200×200) и размещение в `data/brand/` — см. `docs/brand-guide.mdc`
- [ ] Разработать брендинг: логотип для аватарки Telegram (палитра, светлая/темная версии), экспорт ассетов (SVG/PNG 640×640/512×512/200×200) и размещение в `data/brand/`

## Этап 1. Архитектура системы
- [ ] Спроектировать компоненты: бот-обработчики, доменный слой, сервисы интеграций, слой данных
- [ ] Диаграмма потоков: пользователь → бот → доменные кейсы → БД/кэш → AI/vision → нормализация (через GPT, без пищевых баз) → ответ
- [ ] Определить границы модулей и зависимости (чистая архитектура/портно-адаптерный подход)
- [ ] Определить форматы контрактов: DTO/события/ошибки
- [ ] Архитектура Telegram Web Apps: фронтенд (React/Vite/Next), верификация `initData`, авторизация и обмен токенами, обмен данными (REST/WebSocket)
...
- [ ] TDEE по уровню активности, дефицит/профицит под цель (темп изменения веса)
- [ ] Конвертация цели по % жира в целевой вес (оценка без/с учетом LBM)
- [ ] Распределение БЖУ (по умолчанию: белок г/кг LBM, жир %, углеводы остаток)
- [ ] Пересчет дневных бюджетов при изменениях профиля/веса/целей

## Этап 4. Каркас Telegram-бота
- [ ] Подключение к Telegram Bot API, вебхуки/поллинг (для локальной разработки — поллинг)
- [ ] Базовые команды: `/start`, `/help`, `/profile`, `/addmeal`, `/photo`, `/coach`, `/stats`, `/settings`
- [ ] Стейт-машина диалогов, middlewares (логирование, трассировка, локализация)
- [ ] Локализация (RU как дефолт), форматирование сообщений и кнопок
- [ ] Встроенные Web Apps: кнопки `web_app` в клавиатурах, инициализация и безопасная проверка `initData`
- [ ] Frontend мини-приложения: базовый каркас, роуты для дневника, профиля, прогресса
- [ ] Обмен данными: REST/WS эндпоинты для чтения/записи, синхронизация состояния с ботом
