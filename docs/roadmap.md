...
- [ ] Прием сообщений в чате как есть (без специальных команд)
  - [ ] Текст: нормализация через `/api/normalize`, превью и подтверждение (кнопки «Сохранить/Отменить/Править»)
  - [ ] Фото: принять фото, создать черновик приема, очередь на распознавание (этап 9), ручное заполнение и подтверждение
  - [ ] Аудио/голос: STT (Whisper/OpenAI) → нормализация → подтверждение
  - [ ] Автоопределение типа приёма (завтрак/обед/ужин/перекус) по времени и TZ, с возможностью изменения
  - [ ] Черновики: статус `draft → confirmed`, «Отмена», «Undo/Повторить предыдущий», добавление из избранного/пресетов
  - [ ] Идемпотентность по `update_id/message_id`, дедуп одинаковых сообщений
- [ ] REST дневника: CRUD
  - [ ] GET `/api/meals?date=YYYY-MM-DD&tz=...`, GET `/api/meals/{id}`
  - [ ] POST `/api/meals` (создание приёма с `items`), PATCH `/api/meals/{id}`, DELETE `/api/meals/{id}`
  - [ ] Контракты/коды ошибок задокументированы в `docs/contracts.mdc`
- [ ] Бизнес‑логика
  - [ ] Транзакционное обновление `daily_summary` при любых CRUD, защита от гонок
  - [ ] Строгая проверка значений (масса, единицы, БЖУ), единообразные округления; дружественные сообщения об ошибках
  - [ ] Предпочтения/аллергии/диета из настроек — предупреждения при подтверждении
- [ ] Производительность и кэш
  - [ ] Повторное использование Redis‑кэша нормализации для одинаковых сообщений; ключ = locale+text; метрики hit/miss
- [ ] Логи и метрики
  - [ ] Сквозной trace_id; метрики: доля авто‑подтверждений, среднее число позиций, время нормализации
- [ ] WebApp (дневник)
  - [ ] Список приёмов за день, просмотр позиций, редактирование/удаление
  - [ ] Быстрое добавление/редактирование позиций (тонкие UI для сложных блюд), избранное/пресеты, фильтры по датам
...
- [ ] Хранилище: S3/MinIO, sha256‑дедуп, превью/thumbnail
...
- [ ] Предпочтения/аллергии/диет‑режим; предупреждения при подтверждении приёмов
