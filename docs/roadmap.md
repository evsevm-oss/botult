...
- [x] Прием фото/альбомов, привязка к приему и времени; мульти‑фото (медиагруппа) → один приём — MVP: одиночные фото, медиагруппы принимаются как несколько кадров; группировка запланирована
...
- [x] Диалог подтверждения: список блюд → выбор/корректировка → сохранение; фолбэк на ручной ввод — MVP: команды `/checkphoto` и `/savephoto`
- [x] Перформанс/стоимость: кэширование vision/LLM, лимиты размера/вызовов, учет стоимости на пользователя/день — Redis‑кэш по sha256, ограничение изображений до 1600px
