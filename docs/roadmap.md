...
- [x] Обработка неопределенности: запрос уточнений у пользователя, варианты выбора — флаг `needs_clarification` (расширим на GPT позже)
- [x] Кэширование результатов и частых блюд; пресеты пользовательских блюд — (каркас) Redis‑клиент подключен, кэш добавим при интеграции GPT
