...
- [ ] Фото (Vision)
  - [ ] Приём фото и альбомов (медиагруппа) → объединение в один приём
  - [ ] Предобработка изображений (ориентация/resize/сжатие), сохранение в объектном хранилище
  - [ ] Очередь и воркер распознавания; дедупликация по sha256; лимиты на пользователя/день
  - [ ] Вызов OpenAI Vision (gpt-4o/4o-mini) с стабильным JSON (response_format=json)
  - [ ] Эвристики порций (portion_priors), учёт метода готовки/масла; QC (энергосогласованность)
  - [ ] Превью в боте с БЖУ и кнопками «Сохранить / Отменить / Дополнить»
  - [ ] Идемпотентность по update_id/message_id и корректная привязка к чату/сообщению
  - [ ] Обновление daily_summary и таймзона CEST при сохранении
- [ ] Голос (STT → нормализация)
  - [ ] Загрузка voice, распознавание через Whisper/gpt-4o-mini-transcribe (RU)
  - [ ] Нормализация текста 2‑шаговым LLM (food check → items/macros), те же правила, что для текста
  - [ ] Поддержка фраз вида «Запиши…», «Добавь в дневник…» и прямого описания блюда
  - [ ] Превью + сохранение как для текста/фото
- [ ] Комбинированные случаи
  - [ ] Объединение фото+текст (naive fusion): приоритет текста над vision, уточнения
  - [ ] Единый черновик (Redis) и быстрые опции уточнений
- [ ] Интеграция и надёжность
  - [ ] Метрики (вызовы, стоимость, доля уточнений), структурные логи и trace_id
  - [ ] Обработка ошибок/таймаутов провайдера, понятные сообщения пользователю
