...
- [x] WebApp: добавить нижний блок «Настройки» (единый UX‑блок в стиле остальных карточек; адаптив, доступность, состояния загрузки/ошибок)
- [x] WebApp: в блоке «Настройки» реализовать поле «ID специалиста» (подключение тренера/диетолога для наблюдения за данными)
- [x] Единицы измерения (кг/фунты, мл/унции), часовой пояс/TZ, локаль
- [x] Предпочтения/аллергии/диет‑режим; предупреждения при подтверждении приёмов — инициализировано: `/api/settings`
- [x] Частота уведомлений; времена приемов пищи
- [x] Избранные блюда/рецепты, пользовательские блюда (композиции)
- [x] Настройки рассылок: частота, часы, типы контента, opt‑in/out на видео

## Этап 15. Добавление еды по фото и голосу (Vision + STT)
- [x] Фото (Vision)
  - [x] Приём фото и альбомов (медиагруппа) → объединение в один приём
  - [x] Предобработка изображений (ориентация/resize/сжатие), сохранение в объектном хранилище
  - [x] Очередь и воркер распознавания; дедупликация по sha256; лимиты на пользователя/день
  - [x] Вызов OpenAI Vision (gpt-4o/4o-mini) с стабильным JSON (response_format=json)
  - [x] Эвристики порций (portion_priors), учёт метода готовки/масла; QC (энергосогласованность)
  - [x] Превью в боте с БЖУ и кнопками «Сохранить / Отменить / Дополнить»
  - [x] Идемпотентность по update_id/message_id и корректная привязка к чату/сообщению
  - [x] Обновление daily_summary и таймзона CEST при сохранении
- [x] Голос (STT → нормализация)
  - [x] Загрузка voice, распознавание через Whisper/gpt-4o-mini-transcribe (RU)
  - [x] Нормализация текста 2‑шаговым LLM (food check → items/macros), те же правила, что для текста
  - [x] Поддержка фраз вида «Запиши…», «Добавь в дневник…» и прямого описания блюда
  - [x] Превью + сохранение как для текста/фото
- [x] Комбинированные случаи
  - [x] Объединение фото+текст (naive fusion): приоритет текста над vision, уточнения
  - [x] Единый черновик (Redis) и быстрые опции уточнений
- [ ] Интеграция и надёжность
  - [x] Метрики (вызовы, стоимость, доля уточнений), структурные логи и trace_id
  - [x] Обработка ошибок/таймаутов провайдера, понятные сообщения пользователю
  - [ ] Регресc‑чеклисты: одиночное фото, медиагруппа, голос, комбинированный ввод

## Этап 16. Качество данных и наблюдаемость (без тестов)
...
## Этап 17. Производительность и стоимость
...
## Этап 18. Документация разработчика
...
## Этап 19. Готовность к продукту (без деплоя/тестов)
...
## Этап 20. Контент: сообщения, видео и рассылки
...
## Этап 21. Брендинг: дизайн-система
