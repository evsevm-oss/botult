...
- [ ] Интеграция UI с API/БД
  ...
  - [ ] Вес: `GET/POST /api/weights`, пересчёт бюджетов; MA7/медиана и фильтр выбросов на графике
  - [ ] Доля жира: серверный расчёт и сохранение замеров (`POST /api/bodyfat/estimate`, `POST /api/bodyfat`)
  - [ ] Тренды/сводки: `GET /api/summary/{daily,weekly,monthly}`, `GET /api/trends`, `GET /api/compliance`, `GET /api/alerts`
- [ ] Фильтры, TZ и экспорт
  - [ ] Фильтры периода (неделя/месяц/квартал/год) и таймзоны; корректные границы даты
  - [ ] Скачивание CSV (например, `/api/summary/weekly.csv`) из WebApp
- [ ] Интеграция с ботом
  - [ ] Кнопка `web_app` в меню/клавиатуре бота, deeplink и передача параметров старта
  - [ ] CTA из сообщений/рассылок → открытие нужного экрана WebApp
- [ ] UX, доступность и офлайн
  - [ ] Скелетоны/состояния загрузки, обработка офлайн/ошибок, aria‑атрибуты
  - [ ] Локализация RU как дефолт, подготовка к i18n
- [ ] Рассылки
  - [ ] Недельные дайджесты (текст/карусели/видео) с CTA на WebApp, персонализация по данным сводок
