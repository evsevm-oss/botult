...
- [ ] Прием фото/альбомов, привязка к приему и времени; мульти‑фото (медиагруппа) → один приём
  - [x] Приём одиночных фото; медиагруппы принимаются по одному (свод в один приём будет добавлен позже)
- [x] Предобработка: ресайз/компрессия/ориентация/EXIF; безопасный контент‑фильтр (частично: предобработка)
- [x] Пайплайн: очередь (Redis), статусы `queued → processing → ready/failed`; воркер‑заглушка
...
- [x] Диалог подтверждения: MVP — команды `/checkphoto <image_id>` и `/savephoto <image_id>` для подтверждения и сохранения
